...
if ( ! class_exists( 'Paralog' ) ) {
	...
	class Paralog
	{
		...
		public function __construct()
		{
			...
			$this->plugin_url = untrailingslashit(
				plugins_url(
		...
		public function paralog_menu()
		{
			...
			if ( function_exists( 'add_menu_page' ) ) {
				add_menu_page(
					__( "Journaux de décollages / treuillés", PL_DOMAIN ),
					__( "Paralog", PL_DOMAIN ), $allowed_group,
					PL_ADMIN_SLUG,
					array( $this, 'about' ),
				...
				if ( function_exists( 'add_submenu_page' ) ) {
					...
					add_submenu_page(
						PL_ADMIN_SLUG,
						__( "À propos de Paralog", PL_DOMAIN ),
						__( "À propos de", PL_DOMAIN ),
						$allowed_group,
						PL_ADMIN_SLUG,
						array( $this, 'about' )
					);
					...
					$logs_hook = add_submenu_page(
						PL_ADMIN_SLUG,
						__( "Décollages Paralog", PL_DOMAIN ),
						__( "Décollages", PL_DOMAIN ),
						$allowed_group,
						'paralog-logs',
						array( $this, 'list_logs' )
					);
					add_action( "load-$logs_hook", array( $this, 'add_options' ) );
					add_submenu_page(
						'paralog-logs',
						__( "Ajouter un décollage / treuillé", PL_DOMAIN ),
						__( "Ajouter un décollage / treuillé", PL_DOMAIN ),
						$allowed_group,
						'paralog-logs-form',
						array( $this, 'form_log' )
					 ...
					 * person
					 */
					$persons_hook = add_submenu_page(
						PL_ADMIN_SLUG,
						__( "Personnes Paralog", PL_DOMAIN ),
						__( "Personnes", PL_DOMAIN ),
						$allowed_group,
						'paralog-persons',
						array( $this, 'list_persons' )
					);
					add_action( "load-$persons_hook", array( $this, 'add_options' ) );
					add_submenu_page(
						'paralog-persons',
						__( "Ajouter une personne", PL_DOMAIN ),
						__( "Ajouter une personne", PL_DOMAIN ),
						$allowed_group,
						'paralog-persons-form',
						array( $this, 'form_person' )
					);
					/*
					 * lines
					 */
					$lines_hook = add_submenu_page(
						PL_ADMIN_SLUG,
						__( "Lignes Paralog", PL_DOMAIN ),
						__( "Lignes", PL_DOMAIN ),
						$allowed_group,
						'paralog-lines',
						array( $this, 'list_lines' )
					);
					add_action( "load-$lines_hook", array( $this, 'add_options' ) );
					add_submenu_page(
						'paralog-lines',
						__( "Ajouter une ligne", PL_DOMAIN ),
						__( "Ajouter une ligne", PL_DOMAIN ),
						$allowed_group,
						'paralog-lines-form',
						array( $this, 'form_line' )
					);
					/*
					 * sites
					 */
					$sites_hook = add_submenu_page(
						PL_ADMIN_SLUG,
						__( "Sites Paralog", PL_DOMAIN ),
						__( "Sites", PL_DOMAIN ),
						$allowed_group,
						'paralog-sites',
					...
					add_submenu_page(
						'paralog-sites',
						__( "Ajouter un site", PL_DOMAIN ),
						__( "Ajouter un site", PL_DOMAIN ),
						$allowed_group,
						'paralog-sites-form',
						array( $this, 'form_site' )
					);
					/*
					 * activities
					 */
					...
					add_submenu_page(
						'paralog-attendance',
						__( "Ajouter une présence", PL_DOMAIN ),
						__( "Ajouter une présence", PL_DOMAIN ),
						$allowed_group,
						'paralog-attendances-form',
						array( $this, 'form_attendance' )
					);
					/*
					 * options
					 */
					add_submenu_page(
						PL_ADMIN_SLUG,
						__( 'Réglages', PL_DOMAIN ),
						__( 'Réglages', PL_DOMAIN ),
						'manage_options',
						'paralog_options',
						array( $this, 'form_options' )
					);
				}
			}
		}
		...
		public function list_logs()
		{
			...
			if ( 'delete' === $class->current_action() ) {
				...
				$message = '<div class="updated below-h2" id="message"><p>' .
				           sprintf( __( 'Élément(s) supprimé(s): %d', PL_DOMAIN ), $count ) . '</p></div>';
			} else {
				$message = '';
			}
            ...
            <div class="wrap">
                ...
                <a href="<?php echo get_admin_url(
					get_current_blog_id(),
					"admin.php?page=$page-form&paged=$paged" ); ?>"
