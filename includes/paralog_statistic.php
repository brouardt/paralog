...
class Paralog_Statistic
{
	public static function insert_demo_datas()
	{
		...
		$query = $wpdb->prepare(
			"INSERT INTO `$table` " .
			"(`firstname`, `lastname`, `pilot_type`, `licence`, `winchman`, `winchman_type`, `user_id`, `deleted`) " .
			"VALUES " .
		...
		$query = $wpdb->prepare(
			"INSERT INTO `$table` " .
			"(`site_name`, `line_name`, `winchman_name`, `winchman_type`, " .
			"`pilot_name`, `pilot_type`, `passenger_name`, `total_flying_weight`, `takeoff`, " .
			"`user_id`, `deleted`) " .
			"VALUES " .
		...
		$query = $wpdb->prepare(
			"INSERT INTO `$table` " .
			"(`activity_id`, `date`, `site_name`, `line_name`, " .
			"`start_wind_orientation`, `end_wind_orientation`, " .
			"`start_counter`, `end_counter`, " .
			"`start_time`, `end_time`, " .
			"`start_gazoline`, `end_gazoline`, " .
			"`comment`, `winch_incident`, `fly_incident`, " .
			"`user_id`, `deleted`) " .
			"VALUES " .
			"(1, '2018-04-24', 'Aslonnes \"Le Fort\"', 'Treuil 1B', " .
			"%s, %s, " .
			"64120, 64278, " .
			"'13:25:00', '18:16:00', " .
			"45, 33, " .
		...
		$query = $wpdb->prepare(
			"INSERT INTO `$table` " .
			"(`activity_id`, `person_type`, `person_name`, `user_id`, `deleted`) " .
			"VALUES" .
            ...
            <form name="statistiques" method="get" action="">
				...
				if ( $demo_data == '1' ) {
					echo '<p>' . __( 'aucune donnée à visualiser ?', PL_DOMAIN ) .
					     '<button type="submit" name="demo" value="1" class="page-title-action">'
					     . __( 'ajouter de données de démonstration', PL_DOMAIN ) .
					     '</button></p>';
				}
				if ( ! empty( $param_year ) ) {
					echo '<p>' . __( 'exporter les données sélectionnées' ) .
					     '<button type="submit" name="export" value="1" class="page-title-action">' .
					     __( 'exporter', PL_DOMAIN ) .
					     '</button></p>';
				}
				?>
