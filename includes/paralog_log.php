...
class Paralog_Log extends Paralog_Table
{
	...
	public function column_takeoff( $item )
	{
		...
		return sprintf(
			'%1$s %2$s',
			mysql2date( $this->datetime_format, $item['takeoff'] ),
            ...
            <h1><?php _e( 'Fiche de décollage / treuillé', PL_DOMAIN ); ?>
                <a class="add-new-h2"
                   href="<?php echo get_admin_url(
					   get_current_blog_id(),
					   sprintf(
						   'admin.php?page=paralog-logs&paged=%d',
						   $this->get_pagenum()
					   ) ); ?>">
	...
	public function log_form_meta_box_handler( $item )
	{
        ...
        <table cellspacing="2" cellpadding="5" style="width: 100%;" class="form-table">
            ...
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="winchman_id">
						<?php _e( 'Nom du treuilleur', PL_DOMAIN ); ?>
                    </label>
                </th>
                <td>
            ...
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="pilot_id">
						<?php _e( 'Nom du pilote', PL_DOMAIN ); ?>
                    </label>
                </th>
                <td>
            ...
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="passenger_name">
						<?php _e( 'Nom du passager', PL_DOMAIN ); ?>
                    </label>
                </th>
                <td>
                    <input id="passenger_name" name="passenger_name" type="text" style="width: 95%"
                           value="<?php echo esc_attr( $item['passenger_name'] ); ?>" size="50" maxlength="129"
                           class="code"
            ...
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="total_flying_weight">
						<?php _e( 'Poid total volant (PTV)', PL_DOMAIN ); ?>
                    </label>
                </th>
                <td>
                    <input id="total_flying_weight" name="total_flying_weight" type="text" style="width: 5em"
                           value="<?php echo esc_attr( $item['total_flying_weight'] ); ?>" size="5" maxlength="4"
                           class="code"
            ...
            <tr class="form-field">
                <th valign="top" scope="row">
                    <label for="takeoff_time">
						<?php _e( 'Heure', PL_DOMAIN ); ?>
                    </label>
                </th>
                <td>
		...
		switch ( $column_name ) {
			...
			case 'winchman_name':
				return $item[ $column_name ] . '<br>' .
				       '<span style="font-size: smaller;color: ' . $this->color_person( $item['winchman_type'] ) . '">' .
				       $item['winchman_type'] .
				       '</span>';
			case 'pilot_name':
				return $item[ $column_name ] . '<br>' .
				       '<span style="font-size: smaller;color: ' . $this->color_person( $item['pilot_type'] ) . '">' .
				       $item['pilot_type'] . '</span>';
			default:
